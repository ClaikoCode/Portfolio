...
This section is meant to be updated as this project evolves. I will write down the larger problems/roadblocks I will encounter, the solutions that I found for them, and the knowledge I gained from solving them. It will serve as a log for my development journey and will help me better solidify new information.
...
* Interestingly, Typescript does not allow file extensions to be use during import. Apparently this is due to how the Typescript compiler evaluates file types. It can be fixed with a Typescript setting but I chose to comply with the system, even though I don't fully agree with it. 
...
I also added an optional property in IconProps for size, which will add an inline style in the JSX element specifying the width. This allows specific size differences to be applied to Icons if needed.

## Day 6

Removed the size property for icons in favor of a CSS-based approach that I would have used had I known about it earlier. The project now uses CSS descendants to determine the size of the icons when needed. I lose the functionality of individually controlling icon sizes but as the more common use case for my project is to control the size of all icons pertaining to a certain section, the new solution fits much better. This solution is much cleaner in my opinion as these icons are meant to be static objects and have no real reason to be controlled on the Javascript side. A specific "size" value is instead hardcoded in the CSS but this would have been the case either way. The value would instead have been hardcoded in a TSX file.
...
I also updated several elements to have better spacing and improved consistency of padding and margin usage. I noticed I had a more intuitive understanding of the box model when implementing these adjustments as I was often able to replicate whatever vision I had in my head in CSS code, which I was happily surprised about.

## Day 10

I improved my color palette "system" further by borrowing some knowledge from my shader programming experience. Linear interpolation is an extremely useful tool in many ways but it is used extensively when mixing colors together. I wanted to have an equivalent of this as I had defined a lighter version of my primary and a darker version of my secondary that was simply picking a color value that was brighter or darker in the picking box. Whilst this is not a problem as there are only two colors to handle, I still wanted a better solution. 

That introduced me to the color-mix() function in CSS. I now mix the base color with white or black by a certain percentage to make it brighter or darker, which worked! The four colors in my palette are now fully responsible of determining the look of the page. All other color variables are now relative to that base palette. This relative offset of colors might not work properly if I want to have a light-theme version of the website, but that is not part of my goal currently.

## Day 11

I wanted to improve the experience section by structuring it in a way that tells my experience. Firstly, I dropped two stylings depending on wordmark or icon in favor of a single styling that works for both of them. I felt there was no real need to separate them like that and I could then more easily structure each row exactly how I wanted it to be. 

I recently saw a video on youtube that showcased background gradients in CSS. I got inspired by this and decided to try it myself. I wanted the experience section to have a visual representation of my confidence and experience in each technology/language. I chose to achieve this by creating a quickly falling gradient that would be bright at the top and taper off to the normal element background color. The point is to bring attention to these top elements and visually represent my confidence in them being displayed.

I have also added a lot of "real" text to the site, meaning I have replaced place-holder Lorem Ipsum with actual projects, descriptions about each project, and description about myself and my experiences. This will most likely be edited in the future. Above all else, the text is quite long and can be difficult to absorb. This is a common problem that occurs with CV, presentations, research papers, etc. The solution I am thinking of is to instead of listing courses and aspects of each project in text, I should do it visually and separately so it is more clear. I could have an element that simply shows a list of items where I can write these lists instead. This would highlight them, which draws attention to them, and it would also make them more easily digestible.
